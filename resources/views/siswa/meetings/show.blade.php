                ...
                <div class="content-card-body">
                    @forelse($meeting->assignments as $a)
                        <div class="p-4 mb-4 rounded border-start border-4 border-warning" style="background-color: var(--bg-body); border-radius: var(--radius-md) !important;">
                            ...
                            @else
                                <form method="POST" action="{{ route('siswa.assignments.submit', $a) }}" enctype="multipart/form-data" onsubmit="var btn = this.querySelector('button[type=submit]'); if (btn) { setTimeout(function(){ btn.disabled = true; btn.innerHTML = '<i class=\'fas fa-spinner fa-spin me-1\'></i> Mengirim...'; }, 10); }">
                                    @csrf
                                    <div class="mb-3">
